...
(function (RunConfigurationType) {
    ...
    RunConfigurationType["Cgen"] = "Cgen";
...
var RunConfigurationManager = /** @class */ (function () {
    ...
    Object.defineProperty(RunConfigurationManager.prototype, "configuration", {
        get: function () {
    ...
    RunConfigurationManager.prototype.setIconForV = function (runConfigurationType) {
        var icon = runIcons;
        if (runConfigurationType == RunConfigurationType.Test) {
            icon = testIcons;
        }
...
var RetrieveCodeResult = /** @class */ (function () {
    function RetrieveCodeResult(output) {
        this.output = output;
    }
...
var CodeRunner = /** @class */ (function () {
    ...
    CodeRunner.retrieveCgenCode = function (code) {
        var data = new FormData();
        data.append("code", code);
        return fetch("/cgen", {
            method: "post",
            ...
            .then(function (hash) { return new RetrieveCodeResult(hash); });
    };
...
var Editor = /** @class */ (function () {
    function Editor(id, wrapper, repository, readOnly, mode) {
        var _this = this;
        var editorConfig = {
            mode: mode,
            lineNumbers: true,
        ...
        this.textAreaElement = wrapper.querySelector("textarea.".concat(id));
    ...
    Editor.prototype.hide = function () {
        var realEditorElement = this.textAreaElement.nextSibling;
        console.log(realEditorElement);
        if (realEditorElement !== undefined) {
            realEditorElement.style.display = "none";
        }
        this.wrapperElement.classList.add("single-editor");
    };
    Editor.prototype.show = function () {
        ...
        console.log(realEditorElement);
        if (realEditorElement !== undefined) {
            realEditorElement.style.display = "block";
        }
        (_b = (_a = this.wrapperElement.querySelector('.playground__editor')) === null || _a === void 0 ? void 0 : _a.classList) === null || _b === void 0 ? void 0 : _b.remove("single-editor");
    };
...
var Playground = /** @class */ (function () {
    ...
    function Playground(editorElement) {
        ...
        this.editor = new Editor("main", editorElement, this.repository, false, "v");
        this.cgenEditor = new Editor("cgen", editorElement, new TextCodeRepository(""), true, "text/x-csrc");
    ...
    Playground.prototype.run = function () {
        var configuration = this.runConfigurationManager.configuration;
    ...
    Playground.prototype.retrieveCgenCode = function () {
        ...
        CodeRunner.retrieveCgenCode(code)
            ...
            var resultCode = filteredLines.join("\n");
            var v2c = {};
            for (var mappingKey in mapping) {
                // @ts-ignore
                var line = mapping[mappingKey];
                var chenIndex = filteredLines.indexOf(line);
                if (chenIndex != -1) {
                    // @ts-ignore
                    v2c[mappingKey] = chenIndex;
                }
            }
            var mainIndex = filteredLines.indexOf("void main__main(void) {");
            console.log(v2c);
            window.localStorage.setItem("cgen-mapping", JSON.stringify(v2c));
            _this.clearTerminal();
            _this.cgenEditor.show();
            _this.cgenEditor.setCode(resultCode);
            _this.cgenEditor.editor.scrollIntoView({ line: mainIndex, ch: 0 });
            _this.closeTerminal();
        })
            .catch(function (err) {
            console.log(err);
            _this.writeToTerminal("Can't compile and get C code. Please try again.");
        });
    };
    ...
    Playground.prototype.setupShortcuts = function () {
        ...
        this.editor.editor.on("mousedown", function (instance, e) {
            setTimeout(function () {
                var _a;
                var prevHighlightedLine = window.localStorage.getItem("highlighted-line");
                if (prevHighlightedLine != undefined) {
                    _this.cgenEditor.editor.removeLineClass(parseInt(prevHighlightedLine), "text", "cgen-highlight");
                }
                var cursor = instance.getCursor();
                var line = cursor.line + 1;
                var mappingString = (_a = window.localStorage.getItem("cgen-mapping")) !== null && _a !== void 0 ? _a : "{}";
                var mapping = JSON.parse(mappingString);
                var cgenLine = mapping[line];
                if (cgenLine === undefined) {
                    return;
                }
                _this.cgenEditor.editor.scrollIntoView({ line: cgenLine + 20, ch: 0 });
                console.log(cgenLine);
                _this.cgenEditor.editor.addLineClass(cgenLine, "text", "cgen-highlight");
                window.localStorage.setItem("highlighted-line", cgenLine.toString());
                _this.editor.editor.focus();
            }, 100);
        });
    ...
    Playground.prototype.closeTerminal = function () {
