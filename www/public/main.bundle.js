...
`;var _=class{themes=[new v,new A];currentTheme=null;onChange=[];queryParams;changeThemeButton=null;predefinedTheme=null;fromQueryParam=!1;constructor(e,t=null){this.queryParams=e,this.predefinedTheme=t,this.changeThemeButton=document.querySelector(".js-playground__action-change-theme")}registerOnChange(e){this.onChange.push(e)}loadTheme(){let e=this.queryParams.getURLParameter(_.QUERY_PARAM_NAME);if(e!=null){this.fromQueryParam=!0;let n=this.findTheme(e);this.turnTheme(n);return}let t=window.localStorage.getItem(_.LOCAL_STORAGE_KEY);if(t!=null){let n=this.findTheme(t);this.turnTheme(n);return}if(this.predefinedTheme!==null&&this.predefinedTheme!==void 0){this.turnTheme(this.predefinedTheme);return}this.turnTheme(new v)}findTheme(e){let t=this.themes.filter(a=>a.name()===e),n=t[0];return t.length==0&&(n=new v),n}turnTheme(e){this.currentTheme=e,this.onChange.forEach(a=>a(e));let t=oe;e.name()==="dark"&&(t=re),this.changeThemeButton!==null&&(this.changeThemeButton.innerHTML=t),document.querySelector("html").setAttribute("data-theme",e.name()),this.fromQueryParam||window.localStorage.setItem(_.LOCAL_STORAGE_KEY,e.name()),this.fromQueryParam&&this.queryParams.updateURLParameter(_.QUERY_PARAM_NAME,e.name())}turnDarkTheme(){this.turnTheme(new v)}turnLightTheme(){this.turnTheme(new A)}toggleTheme(){this.currentTheme&&(this.currentTheme.name()==="light"?this.turnDarkTheme():this.turnLightTheme())}},x=_;h(x,"QUERY_PARAM_NAME","theme"),h(x,"LOCAL_STORAGE_KEY","theme");var G=class{selectElement;onSelectHandler=null;constructor(){this.selectElement=document.querySelector(".js-examples__select")}registerOnSelectHandler(e){this.onSelectHandler=e}mount(){if(this.selectElement===null||this.selectElement===void 0)return;let e=this.selectElement.querySelector(".dropdown__list"),t=this.selectElement.querySelector(".dropdown__button");e!==null&&t!==null&&(B.forEach(function(d,g){e.innerHTML+=G.exampleElementListTemplate(d.name,g)}),t.innerHTML=B[0].name);let n=this.selectElement.querySelectorAll(".dropdown__list-item");n.forEach(d=>{d.addEventListener("click",()=>{let g=d.innerText,m=B.find(b=>b.name===g);this.onSelectHandler!==null&&m&&this.onSelectHandler(m)})});let a=this.selectElement.querySelector(".dropdown__button"),u=this.selectElement.querySelector(".dropdown__list"),l=this.selectElement.querySelector(".dropdown__input_hidden");a.addEventListener("click",function(){u.classList.toggle("dropdown__list_visible"),this.classList.toggle("dropdown__button_active")}),n.forEach(function(d){d.addEventListener("click",function(g){n.forEach(function(b){b.classList.remove("dropdown__list-item_active")}),g.target.classList.add("dropdown__list-item_active"),a.innerText=this.innerText,l.value=this.dataset.value??"",u.classList.remove("dropdown__list_visible")})}),document.addEventListener("click",function(d){d.target!==a&&(a.classList.remove("dropdown__button_active"),u.classList.remove("dropdown__list_visible"))}),document.addEventListener("keydown",function(d){(d.key==="Tab"||d.key==="Escape")&&(a.classList.remove("dropdown__button_active"),u.classList.remove("dropdown__list_visible"))})}},O=G;h(O,"exampleElementListTemplate",function(e,t){let n="";return t===0&&(n="dropdown__list-item_active"),`
